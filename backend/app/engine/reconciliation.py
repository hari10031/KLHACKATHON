...
class ReconciliationEngine:
    ...
    def get_all_gstins(self) -> List[str]:
        ...
        try:
            ...
            return [r["gstin"] for r in result if r.get("gstin")]
        except Exception as exc:
            logger.error(f"Failed to fetch GSTIN list: {exc}")
            return []
    ...
    def get_return_periods(self) -> List[str]:
        ...
        try:
            result = execute_query("""
                ...
                ORDER BY period
            """)
            return [r["period"] for r in result if r.get("period")]
        except Exception as exc:
            logger.error(f"Failed to fetch return periods: {exc}")
            return []
