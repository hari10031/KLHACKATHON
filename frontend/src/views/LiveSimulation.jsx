...
import {
    ...
    TrendingUp, GitBranch, AlertCircle, RefreshCw
} from 'lucide-react'
import { runSimulation, uploadSimulation } from '../api'

const STEP_LABELS = [
    { label: 'Upload Data', icon: Upload, desc: 'Ingest CSV or demo dataset' },
...
export default function LiveSimulation() {
    ...
    const animateGraph = useCallback((graph) => {
        ...
        setVisibleNodes([]); setVisibleEdges([])
    ...
    useEffect(() => {
        ...
        nodes.forEach((node, i) => {
            ...
            nodePositions[node.id] = { x: hw + radius * Math.cos(angle), y: hh + radius * Math.sin(angle) }
        })
        ...
        edges.forEach(edge => {
            ...
            if (!edge.is_circular) ctx.setLineDash([4, 4])
            ...
            ctx.lineTo(mx - 6 * Math.cos(angle + Math.PI / 6), my - 6 * Math.sin(angle + Math.PI / 6))
            ctx.fillStyle = edge.is_circular ? '#ef4444' : '#94a3b8'; ctx.fill()
        ...
        nodes.forEach(node => {
            ...
            if (node.risk_level === 'critical' || node.risk_level === 'high') {
                ctx.beginPath(); ctx.arc(pos.x, pos.y, r + 6, 0, 2 * Math.PI)
                ctx.fillStyle = node.risk_level === 'critical' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)'; ctx.fill()
            }
            ctx.beginPath(); ctx.arc(pos.x, pos.y, r, 0, 2 * Math.PI)
            const nodeColor = node.type === 'phantom' ? '#6b7280' : node.risk_level === 'critical' ? '#ef4444' : node.risk_level === 'high' ? '#f59e0b' : node.risk_level === 'medium' ? '#14b8a6' : '#22c55e'
            ctx.fillStyle = nodeColor; ctx.fill()
            ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke()
            ctx.font = 'bold 10px Inter, sans-serif'; ctx.fillStyle = '#1e293b'; ctx.textAlign = 'center'
    ...
    const reset = () => {
        ...
        setVisibleNodes([]); setVisibleEdges([]); setSelectedFraud(null)
        ...
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                        ...
                        Live Simulation &mdash; Real-Time Fraud Detection
                    </h2>
                ...
                {step > 0 && (
                    <button onClick={reset}
                        className="flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs font-semibold text-slate-600 transition-colors">
                        ...
                        return (
                            <React.Fragment key={i}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isDone ? 'bg-green-100 text-green-600' : isActive ? 'bg-teal-100 text-teal-600 ring-2 ring-teal-300 animate-pulse' : 'bg-slate-100 text-slate-400'}`}>
                                        {isDone ? <CheckCircle size={18} /> : <Icon size={18} />}
                                    </div>
                                    <div>
                                        <p className={`text-xs font-semibold ${isDone ? 'text-green-700' : isActive ? 'text-teal-700' : 'text-slate-400'}`}>{label}</p>
                                        <p className="text-[9px] text-slate-400">{desc}</p>
                                    </div>
                                </div>
                                {i < 3 && <ArrowRight size={14} className="text-slate-200 mx-1" />}
                            </React.Fragment>
                        )
                    })}
                </div>
            ...
            {step === 0 && !loading && (
                <div className="grid grid-cols-2 gap-4">
                    {/* Demo Card */}
                    <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-xl border border-teal-200 p-6 hover:shadow-lg transition-all cursor-pointer group" onClick={handleDemo}>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-12 h-12 rounded-xl bg-teal-100 group-hover:bg-teal-200 flex items-center justify-center transition-colors">
                                <Play size={22} className="text-teal-700 ml-0.5" />
                            </div>
                            <div>
                                <h3 className="text-base font-bold text-teal-900">Run Demo Simulation</h3>
                                <p className="text-xs text-teal-600 mt-0.5">3 companies · ~80 invoices · circular trades + phantom invoices</p>
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            {[
                                ...
                                <div key={c.gstin} className="flex items-center gap-2 bg-white/70 rounded-lg px-3 py-2 border border-teal-100">
                        ...
                        <div className="mt-4 text-xs font-semibold text-teal-700 flex items-center gap-1">
                            Click to run <ArrowRight size={12} />
                        </div>
                    </div>

                    {/* Upload Card */}
                    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl border border-purple-200 p-6 hover:shadow-lg transition-all">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-12 h-12 rounded-xl bg-purple-100 flex items-center justify-center">
                                <Upload size={22} className="text-purple-600" />
                            </div>
                            <div>
                                <h3 className="text-base font-bold text-purple-900">Upload Your Dataset</h3>
                                <p className="text-xs text-purple-600 mt-0.5">CSV with GSTIN, invoice_id, buyer, seller, value, tax, ITC</p>
                            </div>
                        </div>
                        <div className="bg-white/60 rounded-xl border-2 border-dashed border-purple-200 p-6 text-center hover:border-purple-400 transition-colors cursor-pointer"
                            onClick={() => fileInputRef.current?.click()}>
                            <Upload size={28} className="text-purple-300 mx-auto mb-2" />
                            <p className="text-xs font-semibold text-purple-600">Click to upload CSV file</p>
                        ...
                        <div className="mt-3 bg-white/50 rounded-xl p-2.5 border border-purple-100">
            ...
            {loading && (
                <div className="card p-14 text-center">
                    <div className="w-12 h-12 border-[3px] border-teal-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" />
                    <p className="text-sm font-semibold text-slate-700">
                        {step === 1 ? 'Ingesting data & building knowledge graph...' : 'Running fraud detection algorithms...'}
                    </p>
                    <p className="text-[10px] text-slate-400 mt-1.5">This typically takes 2–5 seconds</p>
                </div>
            )}
            ...
            {step === 3 && result && (
                <>
                    {/* Summary Banner */}
                    <div className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 rounded-xl p-5 text-white shadow-xl border border-slate-700">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                <Zap size={18} className="text-teal-400" />
                                <span className="text-sm font-bold">Simulation Complete</span>
                                <span className="badge bg-teal-500/20 text-teal-300 border-teal-500/30">LIVE</span>
                            </div>
                        ...
                        <div className="grid grid-cols-7 gap-3">
                            {[
                                { label: 'Invoices', value: result.invoice_count, danger: false },
                                { label: 'Companies', value: result.companies?.length || 0, danger: false },
                                { label: 'Frauds Found', value: result.fraud_count, danger: true },
                                { label: 'Circular Trades', value: result.summary?.circular_trades || 0, danger: false },
                                { label: 'Phantom Inv.', value: result.summary?.phantom_invoices || 0, danger: false },
                                { label: 'ITC Overclaims', value: result.summary?.itc_overclaims || 0, danger: false },
                                { label: 'ITC at Risk', value: fmt(result.summary?.total_itc_at_risk), danger: true },
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="col-span-2 card p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                                    ...
                                    {animatingGraph && <span className="text-[9px] text-teal-500 animate-pulse ml-1">● Building...</span>}
                                </h3>
                            ...
                            <canvas ref={canvasRef} className="w-full rounded-xl bg-slate-50 border border-slate-100" style={{ height: 380 }} />
                            <div className="mt-2 flex items-center gap-3 text-[9px] text-slate-400">
                                <span className="flex items-center gap-1"><div className="w-6 h-0.5 bg-red-500" /> Circular trade</span>
                                <span className="flex items-center gap-1"><div className="w-6 h-0.5 bg-slate-300 border-dashed" /> Normal edge</span>
                                <span>Node number = risk score %</span>
                            </div>
                        </div>
                        ...
                        <div className="card p-4 max-h-[500px] overflow-y-auto">
                            <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                                ...
                                Detected Cases ({result.frauds?.length || 0})
                            </h3>
                                    ...
                                    return (
                                        <div key={fraud.id || i}
                                            ...
                                            <div className="flex items-center gap-2 mb-1.5">
                                                <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: (FRAUD_COLORS[fraud.type] || '#64748b') + '18' }}>
                                                    <FraudIcon size={13} style={{ color: FRAUD_COLORS[fraud.type] || '#64748b' }} />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className={`text-[9px] font-semibold ${fraud.severity === 'CRITICAL' ? 'text-red-600' : fraud.severity === 'HIGH' ? 'text-orange-600' : 'text-amber-600'}`}>
                                                        {fraud.severity} · {(fraud.confidence * 100).toFixed(0)}% confidence
                                                    </p>
                                                </div>
                                                <span className="text-xs font-bold text-red-700">{fmt(fraud.itc_at_risk)}</span>
                                            </div>
                                            {isSelected && (
                                                <div className="mt-2 bg-white rounded-xl p-2.5 border border-slate-100 text-[10px] text-slate-600 leading-relaxed animate-fade-in">
                                                    ...
                                                    <div className="mt-2 bg-slate-50 rounded-lg p-1.5 text-[9px]">
                                                        <p className="font-semibold text-slate-500 mb-0.5">Recommended Action:</p>
                                                        <p className="text-slate-600">
                                                            {fraud.type === 'CIRCULAR_TRADE' && 'Flag all entities for simultaneous audit. Block ITC under circular invoices. Issue SCN under Section 74.'}
                    ...
                    <div className="card p-5">
                        <h3 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                            <Shield size={15} className="text-teal-600" /> Entity Risk Assessment
                        </h3>
                        <div className="grid grid-cols-3 gap-4">
                            {(result.risk_scores || []).map((rs) => {
                                const riskColor = rs.risk_level === 'critical' ? 'red' : rs.risk_level === 'high' ? 'amber' : rs.risk_level === 'medium' ? 'teal' : 'green'
                                const colors = {
                                    red: { bg: 'bg-red-50', border: 'border-red-200', badgeCls: 'badge badge-critical', bar: 'bg-red-500' },
                                    amber: { bg: 'bg-amber-50', border: 'border-amber-200', badgeCls: 'badge badge-medium', bar: 'bg-amber-500' },
                                    teal: { bg: 'bg-teal-50', border: 'border-teal-200', badgeCls: 'badge bg-teal-100 text-teal-700 border-teal-200', bar: 'bg-teal-500' },
                                    green: { bg: 'bg-green-50', border: 'border-green-200', badgeCls: 'badge badge-low', bar: 'bg-green-500' },
                                }
                                ...
                                return (
                                    <div key={rs.gstin} className={`${c.bg} border ${c.border} rounded-xl p-4`}>
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <span className={c.badgeCls}>{rs.risk_level}</span>
                                        </div>
                                        <div className="mb-3">
                                            <div className="flex justify-between text-xs mb-1.5">
                                                <span className="text-slate-500">Risk Score</span>
                                                <span className="font-bold font-mono text-slate-700">{(rs.risk_score * 100).toFixed(0)}%</span>
                                            </div>
                    ...
                    <div className="bg-gradient-to-br from-indigo-50 to-purple-50/50 rounded-xl border border-indigo-100 p-5">
                        <h3 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                            <Eye size={15} className="text-indigo-500" /> Real-World Use Cases from This Simulation
                        </h3>
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                {
                                    title: 'GST Department Intervention', icon: Shield,
                                ...
                                {
                                    title: 'ITC Blocking (Rule 86A)', icon: XCircle,
                                ...
                                {
                                    title: 'Audit Case Generation', icon: FileText,
                                ...
                                <div key={title} className="bg-white rounded-xl border border-indigo-100 p-4 shadow-sm">
                                    <div className="flex items-center gap-2 mb-3">
